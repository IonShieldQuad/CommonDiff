...
public class GraphDisplay extends JPanel {
    ...
    private void drawGraph(Graphics g) {
        ...
        for (int i = 0; i < graphWidth(); i++) {
            ...
            val = new PointDouble(val.getX(), getPointValue(val.getX()));
    ...
    private double getPointValue(double x) {
        if (Double.isNaN(x)) {
            return Double.NaN;
        }
        if (points.size() == 1) {
        ...
        if (x <= points.get(0).getX()) {
            int lo = 1;
            double alpha = (x - points.get(lo - 1).getX()) / (points.get(lo).getX() - points.get(lo - 1).getX());
            return points.get(lo).getY() * alpha + points.get(lo - 1).getY() * (1 - alpha);
        }
        if (x >= points.get(points.size() - 1).getX()) {
            int lo = points.size() - 1;
            double alpha = (x - points.get(lo - 1).getX()) / (points.get(lo).getX() - points.get(lo - 1).getX());
            return points.get(lo).getY() * alpha + points.get(lo - 1).getY() * (1 - alpha);
        }
        int lo = 0;
        ...
        while (lo <= hi /*&& Math.abs(lo - hi) > 1*/) {
            int mid = (int) Math.floor((hi + lo) / 2d);
